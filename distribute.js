...
worker =
{
   ...
   node:              config.node,
   generating_offset: config.generating_offset,
   ...
   doit: function()
   {
      ...
      {
         return worker.storeTransactions();
      }).then(function()
      {
         return worker.storeLastBlock();
      }).then(function()
      {
         return worker.updateGenerating();
      }).then(function()
      {
            ...
            if(block.generator == worker.address)
            {
               ...
               worker.db.query(query).then(function(rows)
               {
                  if(rows.length == 0)
                  {
                     ...
                     var query = "insert into block(id_node, height, fee, mrt, tx_count, timestamp) values(" + worker.id_node + ", " + block.height + ", " + fees + ", " + worker.mrtPerBlock + ", " + block.transactions.length + ", '" + date.toISOString() + "')";
      ...
      function workOnBlock(block)
      {
         return new Promise(function(resolve, reject)
         {
            ...
            worker.db.query(query).then(function(rows)
            {
               ...
               worker.log("block: " + block.height + ", leases: " + rows.length + ", waves: " + block.fee + ", dist: " + sumWaves + ", mrt: " + block.mrt + ", dist: " + sumMrt, worker.log_level_info);
               resolve(earnings);
            }).catch(function(e)
            {
                     ...
                     Promise.all(earningsFinal.map(function(earning){return workOnEarning(earning, distributionId);})).then(function()
                     {
                        ...
                        worker.db.query(query).then(function(result)
                        {
                           ...
                           worker.db.query(query).then(function(rows)
                           {
                              ...
                              worker.log("block fees: " + (blockFees * worker.wavesShare/100) + ", dist waves: " + distWaves + ", block mrt: " + (blockMrt * worker.mrtShare/100) + ", dist mrt: " + distMrt, worker.log_level_info);
   ...
   checkBalance: function()
   {
      ...
      return new Promise(function(resolve, reject)
      {
         ...
         var query = "select sums.waves as waves, sums.mrt as mrt from (select ifnull(sum(if(id_asset = " + worker.asset_waves + ", amount, 0)), 0) as 'waves', ifnull(sum(if(id_asset = " + worker.asset_mrt + ", amount, 0)), 0) as 'mrt' from journal_line where id_journal_line_type = " + worker.journal_line_type_dist + " and id_journal_line_ref is null) as sums";
            ...
            callApi("/addresses/balance/" + worker.address).then(function(data)
            {
               ...
               callApi("/assets/balance/" + worker.address + "/" + worker.asset_mrt_id).then(function(data)
               {
                  var balanceMrt = data.balance;
                        ...
                        worker.log("balance waves: " + balanceWaves + ", pay waves: " + payWaves + ", balance mrt: " + balanceMrt + ", pay mrt: " + payMrt, worker.log_level_info);

                        if(balanceWaves < payWaves || balanceMrt < payMrt)
                        {
   ...
   sendMail: function(subject, text)
   {
      ...
      return new Promise(function(resolve, reject)
      {
         transporter.sendMail(mailOptions, function(error, info)
         {
            if(error)
            {
               worker.log("ERROR: email couldn't be sent: ", worker.log_level_info);
               reject();
            }
            else
            {
